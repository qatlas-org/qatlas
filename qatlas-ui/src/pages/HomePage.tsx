...
import { useApplications, useDashboardStats, useExecutions } from '../hooks/useQueries';
import { useAuth } from '../hooks/useAuth';
import { LoadingState, ErrorState, Card } from '../components/Primitives';
import { ProjectCard } from '../components/ProjectCard';
import {
  buildProjectCards, sortProjectCards, uniqueExecutors,
} from '../lib/portfolio';
...
export function HomePage() {
  ...
  const kpis = dashboardStats
      ? {
        ...
        executionsInRange: dashboardStats.executions,
        passRate: dashboardStats.passRate,
        currentlyRunning: dashboardStats.currentlyRunning,
      }
      : null;
  ...
  const machineData = useMemo(
      () =>
          (dashboardStats?.executionsByMachine ?? []).map((m) => ({
            name: m.machine,
            count: m.executions,
          })),
      [dashboardStats]
  ...
  if (loadingApps || loadingExecs || loadingDashboard) {
    return <LoadingState label="Loading portfolio…" />;
  }

  if (errorApps || errorExecs || errorDashboard) {
    return <ErrorState message="Could not reach the QAtlas backend." />;
  }
